package com.app.engineerapp.Dashboard.ModifierAccounts.ChangePassword;
...
public class PasswordChangeController implements Initializable {
    ...
    Object returnPerson;
    String returCni;
    public void getemailfromothercontroller(String cni, Object personne){
        returCni=cni;
        returnPerson=personne;
    }
    public void handleChange(String pass) {
        String salt=function.salt_gen(10);
        String table=Variables.type.equals("ing")?"responsable":"technicien";
        function.setPassword(table,((Personne)returnPerson).getCni(),function.hashing(pass+salt),salt);
    }
    ...
    private void menuBack(){
        Parent root;
        try {
            FXMLLoader loader=new FXMLLoader(Objects.requireNonNull(HelloApplication.class.getResource("Dashboard/ModifierAccounts/modifierAccounts.fxml")));
            root=loader.load();
            ModifierAccountsController modifierAccountsController=loader.getController();
            modifierAccountsController.getAccounttoModifier(returCni, Variables.chosenCas,returnPerson);
